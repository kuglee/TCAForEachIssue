...
public struct AppView: View {
  ...
  public var body: some View {
    VStack {
      ForEach(self.store.scope(state: \.childStates, action: \.childAction)) {
        ChildView(store: $0)
      }

      if let store = .some(self.store.scope(state: \.childStates, action: \.childAction)) {
        ForEach(store) { ChildView(store: $0) }  // error: Generic struct 'ForEach' requires that 'Store<IdentifiedArray<UUID, ChildFeature.State>, IdentifiedAction<UUID, ChildFeature.Action>>' conform to 'RandomAccessCollection'
      }
    }
  }
}
